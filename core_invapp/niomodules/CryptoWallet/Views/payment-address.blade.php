                            ...
                            @if(data_get($payment, 'meta.ref')=='yes')
                                ...
                                <div id="crypto-paid" class="popup">
                                    <div class="popup-content">
                                        ...
                                        <p>{{ __('If you already paid, please provide us your payment prove to speed up verification procces.') }}</p>
                                        <form class="form" action="#" method="POST" id="foo-deposit" enctype="multipart/form-data">
                                            <div class="form-group">
                                                <div class="form-label">{{ ('Upload Payment Prove') }} <span class="text-danger">*</span></div>
                                                <!--<div class="form-control-wrap">-->
                                                <!--    <input name="reference" type="text" class="form-control " value="" placeholder="{{ __('Enter your reference id / hash') }}">-->
                                                <!--</div>-->
                                                 <div class="form-control-wrap">
                                                    <input name="reference" type="file" class="form-control ">
                                                </div>
                                            </div>
                                            <div id="custommessage"></div>
                                            <ul class="btn-group justify-between align-center gx-4">
                                                <li><input type="button" class="btn btn-primary btn-block" id="Button" onclick="submitform('foo-deposit', 'custommessage')" value="{{ __('Confirm Payment 3') }}"></li>
                                                <li><a href="#" class="link link-btn link-secondary popup-close">{{ __('Close') }}</a></li>
                                            </ul>
                                            <input type="hidden" name="tnx" value="{{ $tranx->tnx }}">
                                            @csrf
                                        </form>
