...
<div class="nk-pps-apps">
    ...
    <form class="nk-pps-form" action="{{ route('deposit.preview.form') }}" id="deposit-amount-form">
        <div class="nk-pps-field-set">
            <div class="nk-pps-field-row row gy-gs">
                   <div class="nk-pps-field-col col-12 col-sm-6">
                    <div class="nk-pps-field form-group">
                        <div class="form-control-group">
                            <div class="form-label-group">
                            <label class="form-label" for="deposit-amount">USD</label>
                        </div>
                            <div class="form-text-hint form-text-hint-lg">
                                <span class="currency">{{ base_currency() }}</span>
                            </div>
                            <input type="text" class="form-control form-control-lg form-control-number prm-amount" id="usd-amount" oninput="calamount()" name="credit_amount" value="" placeholder="0.00">
                            <input type="hidden" id="prm-credit-currency" name="credit_currency" value="{{ base_currency() }}">
                        </div>
                    </div>
                </div>
                <input type="hidden" name="" onchange="calamount()" id="currency_type" value="BTC">
                <div class="nk-pps-field-col col-12{{ (base_currency()==$default['code']) ? '' : ' col-sm-6'  }} prm-fmsa{{ (gss('deposit_amount_base') == 'no') ? ' nk-pps-only' : ''}}">
                    <div class="nk-pps-field form-group">
                        <div class="form-label-group">
                            <label class="form-label" for="deposit-amount">{{ __('Amount to Deposit in coin') }}</label>
                        </div>
                        <div class="form-control-group">
                            @if($cur_count > 1)
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right text-center{{ $cls_dd }}">
                                        ...
                                        <ul class="link-list-plain{{ $cls_ul }}" id="currency-list">
                                            @foreach($currencies as $code => $item)
                                                ...
                                                <input type="hidden" name="" id="{{ $code }}rate" value="{{ $item['rate'] }}">
                                                <li><a class="switch-currency" href="javascript:void(0)" data-switch="prm"  onclick="changerate(this)"  data-currency="{{ $code }}">{{ $code }}</a></li>
                                            @endforeach
                                        </ul>
                                    </div>
                                </div>
                ...
                <div class="nk-pps-field-col col-12 col-sm-6 prm-tora{{ (base_currency()==$default['code']) ? ' hide' : ''  }}" style="display: none">
                    <div class="nk-pps-field form-group">
                        <div class="form-label-group">
                        ...
                        <div class="form-control-group">
                            ...
                            <input type="text" class="form-control form-control-lg form-control-number prm-amount" id="prm-credit" name="credit_amount" value="3" placeholder="0.00">
                            <input type="hidden" id="prm-credit-currency" name="credit_currency" value="{{ base_currency() }}">
                        </div>
                        @if (gss('deposit_amount_base') != 'no')
                        <div class="form-note-group">
    ...
    <script type="text/javascript">
        function calamount(){
        var usd_amount = document.getElementById("usd-amount").value;
        var coin_amount = document.getElementById("prm-amnt");
        var currency_type = document.getElementById("currency_type").value;
        var coin_rate = document.getElementById(currency_type+"rate").value;
        coin_amount.value = coin_rate * usd_amount;
    }

    function changerate(coinname){
        var coin = coinname.getAttribute("data-currency");
        document.getElementById("currency_type").value = coin;
        calamount();
    }
